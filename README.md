...
- コンフィグのホットリロード
- ~~フィードへの通知を抑制するトグル~~ → v1.1.0
  - ~~エラーが発生した場合などはフィードへ通知を流すようにしているが場合によっては不便かもしれない~~
- ~~RSSのタイトルなどをjsonから設定できるように~~ → v1.1.0
...
- v1.1.0(26-02-25)
  - サムネイル情報取得のスイッチを追加、**デフォルト時は無効**(`shouldFetchThumbnail`項目)
  - RSSのタイトルなどをjsonから設定できるように(`rssGenerator`項目)
  - システムからの通知(例:[INFO]サーバーの更新を待っています)を抑制する設定を追加(`shouldSuppressSystemMessage`項目)
